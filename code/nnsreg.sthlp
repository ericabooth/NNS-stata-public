...
{syntab:Generate}
{synopt:{opt gen:erate(newvar)}}create a fitted-value variable for the estimation sample{p_end}
{synoptline}
{p2colreset}{...}
...
{pstd}
The command is most useful when an analyst wants to describe which ranges are relatively flat or steep, inspect where the fitted relationship changes, and check whether those features survive alternative specifications.
In simulation comparisons distributed with the package, {helpb npregress kernel} and {cmd:npregress series} recover smooth conditional means more accurately on average.  In the clustered benchmark, {cmd:nnsreg}'s {cmd:method(ols)} variant nearly matches the lowest mean error and has a narrower distribution across replications.
{cmd:nnsreg} also fits without a bandwidth choice and yields a compact segment summary with named ranges, slopes, and support counts.
...
{phang}
{opt minobs(#)} sets the smallest group the recursive partition keeps splitting; a group is split only if it holds more than {it:#} observations.  The default is {cmd:minobs(8)}.  Larger values produce fewer, longer segments and are a useful sensitivity check alongside {opt order()}.  Because the reported x ranges lie between adjacent regression-point knots, a range can contain fewer than {it:#} observations, or none at all, even though every partition group obeys {opt minobs()}.
...
{phang}
{opt vce(vcetype)} requests a robust or cluster-robust covariance estimate for the least-squares stage.  {it:vcetype} may be {opt robust} or {opt cluster} {it:clustvar}.  It is allowed only with {cmd:method(ols)}, whose coefficients are ordinary least squares on the spline basis; the standard errors remain conditional on the data-derived knots.

{phang}
{opt slopes} reports a table of per-segment slopes, each the cumulative sum of the spline coefficients up to that segment, with delta-method standard errors and the number of observations in the range.  These are the same quantities the manual {helpb lincom} recipe below produces; the table is also stored in {cmd:e(segments)}.  A range with no observations represents interpolation between adjacent knots.
...
{phang}
{opt generate(newvar)} creates a fitted-value variable for the estimation sample.
...
{synopt:{cmd:e(N_clust)}}number of clusters when {cmd:vce(cluster ...)} is specified{p_end}
{synopt:{cmd:e(r2)}}coefficient of determination, 1 - sum(e^2)/sum((y-ybar)^2){p_end}
{synopt:{cmd:e(r2_a)}}adjusted coefficient of determination{p_end}
{synopt:{cmd:e(rmse)}}root mean squared error, sqrt(sum(e^2)/df_r){p_end}
{synopt:{cmd:e(rss)}}residual sum of squares{p_end}
{synopt:{cmd:e(mss)}}model sum of squares{p_end}
{synopt:{cmd:e(F)}}model F statistic; missing for {cmd:method(connect)}{p_end}
{synopt:{cmd:e(ll)}}log likelihood; missing for {cmd:method(connect)}{p_end}
{synopt:{cmd:e(ll_0)}}log likelihood of the constant-only model; missing for {cmd:method(connect)}{p_end}
...
{synopt:{cmd:e(title)}}estimation-command title{p_end}
{synopt:{cmd:e(estat_cmd)}}{cmd:nnsreg_estat}, which rejects unsupported {cmd:estat} calls{p_end}
...
{synopt:{cmd:e(model)}}{cmd:connect} for the connected fit; {cmd:ols} for the least-squares fit{p_end}
...
{synopt:{cmd:e(vce)}}{cmd:ols}, {cmd:robust}, {cmd:cluster}, or {cmd:heuristic}{p_end}
{synopt:{cmd:e(vcetype)}}display label for robust, cluster-robust, or heuristic standard errors, when applicable{p_end}
{synopt:{cmd:e(clustvar)}}cluster variable when {cmd:vce(cluster ...)} is specified{p_end}

{p2col 5 20 24 2: Matrices}{p_end}
{synopt:{cmd:e(b)}}coefficient vector{p_end}
{synopt:{cmd:e(beta)}}standardized coefficient vector{p_end}
...
squares, so its residuals need not average zero.  {cmd:e(rss)}, {cmd:e(mss)},
and {cmd:e(r2_a)} are recomputed from the connected predictions; the
OLS-specific F and log-likelihood statistics are left missing.  For this
method, {cmd:e(vce)} is {cmd:heuristic} and {cmd:e(model)} is {cmd:connect},
so stored metadata does not mislabel the connected fit as OLS.
...
{pstd}
Standard errors for {cmd:method(ols)} are conditional on the data-derived knots.  For {cmd:method(connect)}, the command rescales an OLS spline covariance matrix by the connected fit's residual variance; these are heuristic reference intervals, not estimator-specific sampling intervals or tests.  Use support counts, graphs, comparison models, and option sensitivity checks before making substantive claims.  When prediction accuracy is central, compare {cmd:nnsreg} with {helpb npregress kernel}, splines, and simpler baselines using a common validation metric.

{pstd}
{helpb margins} and {cmd:estat} are not supported: the coefficient names ({cmd:knot1}, {cmd:knot2}, and so on) are spline-basis terms, not variables in the dataset, and the generated basis variables are temporary.  Use {helpb lincom} on the stored coefficients, or the {opt slopes} option and {cmd:e(segments)}, to report marginal slopes.  {cmd:predict} rebuilds the spline basis from {cmd:e(knots)} and scores observations outside the estimation sample; because the terminal segments extrapolate linearly, {cmd:predict} notes when it scores points beyond {cmd:e(xmin)} and {cmd:e(xmax)}.
...
{p 4 8 2}
Viole, F. 2026. NNS: Nonlinear Nonparametric Statistics. R package version 13.1. https://CRAN.R-project.org/package=NNS
