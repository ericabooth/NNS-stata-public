...
*! heteroskedastic, making this a demanding applied illustration.
*!
*! Produces:
*!   - manuscript/fig_moto_fits.pdf/.png : four fits, one method per panel
*!   - manuscript/fig_moto_knots.pdf/.png: fit, knots, and segment-support diagnostic
...
*! true-curve RMSE used with the simulated designs.  npregress kernel is scored
*! through margins at the held-out regressor values; npregress series uses
*! predict, tolerance(100).  nnsreg, regress, and mkspline+regress score held-out
*! rows directly with predict.
clear all
set more off
set scheme sj
adopath ++ "."
...
nnsreg accel time, order(`ORD') method(ols) vce(robust) noplot
...
graph export "fig_moto_fits.pdf", replace
graph export "fig_moto_fits.png", replace width(2200)

// Segment diagnostic: the upper panel shows the fitted curve and knots.  The
// lower panel shows the slope assigned to each x range, its conditional 95%
// interval, and the number of observations in that range.  Thinly supported
// ranges are dashed so the display prompts investigation rather than turning
// every knot into a threshold claim.
estimates restore moto_ols
local klist "`e(knots)'"
matrix S_moto = e(segments)
...
twoway ///
    ...
    legend(order(2 "NNS OLS fit, order(`ORD')") rows(1) size(small) position(5) ///
        region(lcolor(none))) ///
    graphregion(color(white)) xsize(7) ysize(3.8) name(moto_curve, replace) nodraw

preserve
clear
svmat double S_moto, names(col)
gen double x_mid = (x_lo + x_hi) / 2
gen double ci_lo = slope - `tcrit' * se
gen double ci_hi = slope + `tcrit' * se
gen byte sparse = n < 5
gen byte label_slot = mod(_n, 4)
gen str8 n_label = "n=" + string(n, "%2.0f")

twoway ///
    (pcspike slope x_lo slope x_hi if !sparse, lcolor("27 45 85") lwidth(medthick)) ///
    (rcap ci_lo ci_hi x_mid if !sparse, lcolor("27 45 85") lwidth(vthin)) ///
    (pcspike slope x_lo slope x_hi if sparse, lcolor(gs7) lpattern(shortdash) lwidth(medthick)) ///
    (rcap ci_lo ci_hi x_mid if sparse, lcolor(gs7) lpattern(shortdash) lwidth(vthin)) ///
    (scatter slope x_mid if label_slot == 0, msymbol(O) msize(vsmall) mcolor("27 45 85") ///
        mlabel(n_label) mlabsize(vsmall) mlabposition(12) mlabcolor(gs5)) ///
    (scatter slope x_mid if label_slot == 1, msymbol(O) msize(vsmall) mcolor("27 45 85") ///
        mlabel(n_label) mlabsize(vsmall) mlabposition(6) mlabcolor(gs5)) ///
    (scatter slope x_mid if label_slot == 2, msymbol(O) msize(vsmall) mcolor("27 45 85") ///
        mlabel(n_label) mlabsize(vsmall) mlabposition(11) mlabcolor(gs5)) ///
    (scatter slope x_mid if label_slot == 3, msymbol(O) msize(vsmall) mcolor("27 45 85") ///
        mlabel(n_label) mlabsize(vsmall) mlabposition(5) mlabcolor(gs5)), ///
    yline(0, lcolor(gs8) lpattern(dot)) ///
    ytitle("Slope (g per millisecond)", size(small)) ///
    xtitle("Time after impact (milliseconds)", size(small)) ///
    ylabel(-200(50)150, labsize(small) angle(horizontal)) ///
    xlabel(0(20)60, labsize(small)) ///
    legend(order(1 "Slope over range" 2 "95% conditional robust interval" ///
        3 "Range with n < 5") rows(1) size(vsmall) position(6) region(lcolor(none))) ///
    note("Labels report observations in each x range.  Robust intervals condition on the selected knots.", ///
        size(vsmall)) ///
    graphregion(color(white)) xsize(7) ysize(3.8) name(moto_slopes, replace) nodraw
restore

graph combine moto_curve moto_slopes, cols(1) imargin(small) ///
    note("Knots and large local slopes flag ranges for inspection; they are not estimated thresholds.", ///
        size(vsmall)) ///
    graphregion(color(white)) xsize(7) ysize(7.2) name(moto_knots, replace)
graph export "fig_moto_knots.pdf", replace
graph export "fig_moto_knots.png", replace width(2000)

// Diagnostics table
use "moto_diag.dta", clear
format r2 rmse_obs %6.3f
list, noobs
export delimited using "table_moto_diag.csv", replace

// Featured slope table (for a manuscript log excerpt / reference)
di _n "=== per-segment slopes on the featured NNS OLS fit ==="
webuse motorcycle, clear
nnsreg accel time, order(`ORD') method(ols) vce(robust) noplot slopes
...
webuse motorcycle, clear
// Fixed restricted-cubic-spline basis at full-sample percentile knots.  This
// comparator uses only the marginal distribution of time, but its basis is
// fixed before the folds rather than recomputed inside each training fold.
mkspline msp = time, cubic nknots(5)

tempname cv
postfile `cv' str24 method double rmse int rep using "moto_cv.dta", replace

// The kernel estimator is scored at held-out time values through margins.
// The series estimator uses predict, tolerance(100), which permits scoring
// within the endpoint-bounded held-out set even when a training-fold spline
// basis has a narrower internal support.
local methods "linear mksp kernel series conn ols olsmed"
local npfail = 0
local seriesfail = 0
...
// range.  Thus each repeat scores 131 of the 133 observations.  Random folds
// operate at the observation level, so tied time values can appear in both
// training and validation data; the estimand is prediction for another
// reading at a partly familiar time, not prediction at an unseen time value.
...
forvalues rep = 1/`CVREPS' {
    ...
    forvalues k = 1/`KFOLD' {
        ...
        capture {
            quietly npregress series accel time if fold != `k'
            quietly predict double _tmp_series, mean tolerance(100)
            quietly replace p_series = _tmp_series if fold == `k'
            drop _tmp_series
        }
        if _rc local ++seriesfail
    }
...
di as text "npregress series margins failures across all CV folds: `seriesfail'"
assert `npfail' == 0
assert `seriesfail' == 0

use "moto_cv.dta", clear
gen msort = .
local i = 0
foreach m of local methods {
    local ++i
...
replace label = "npregress series"            if method == "series"
