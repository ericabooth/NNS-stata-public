...
program define nnsreg, eclass
    ...
    // vce() re-weights the least-squares covariance and only composes with
    // method(ols); the connect fit posts a rescaled OLS covariance that a
    // sandwich estimator would not compose with, so refuse the combination.
                  ...
                  string scalar fail_name)
{
    ...
    real colvector rp_x, rp_y, unique_priors, rp_order
